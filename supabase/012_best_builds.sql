...
insert into best_builds (brawler_slug, brawler_name, comment) values
  ...
  ('poco', 'Poco', 'Poco est le bourrin par excellence, sa grande zone d''attaque lui permet de charger ses ultimes assez facilement, qui finissent le travail grâce à son pouvoir star. Le second gadget peut vous sauver de nombreux matchups (Corbac, Lou, Otis) mais est inutile dans plusieurs situations. Si vous avez une composition adéquate, vous pouvez changer de pouvoir star, notamment efficace en zone réservée.'),
  ('rosa', 'Rosa', 'Rosa n''est pas à sous-estimer. Si vous arrivez à poser stratégiquement ses buissons grâce à son gadget, vous pouvez retourner la partie à vous tout seul. Son combo pouvoir star et équipements dégâts + vitesse la rend encore plus dangereuse qu''un Bull. Vous pouvez également choisir de jouer une version plus défensive avec l''autre pouvoir star, qui vous soigne dans les buissons, et le gear gadget accéléré pour avoir encore plus de buissons.')
