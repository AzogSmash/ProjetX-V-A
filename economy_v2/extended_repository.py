...
class IndustrialInsightsMixin:
    ...
    def refresh_achievements(self, uid):
        with immediate_transaction(self.database_path) as c:
            ...
            seasonal=(("season_participant","Participer à une saison",c.execute("SELECT count(*) FROM industrial_season_scores WHERE discord_user_id=? AND score>0",(uid,)).fetchone()[0]>0),("season_top10","Top 10 saison",c.execute("SELECT count(*) FROM industrial_season_rewards WHERE discord_user_id=? AND rank<=10",(uid,)).fetchone()[0]>0),("season_top3","Top 3 saison",c.execute("SELECT count(*) FROM industrial_season_rewards WHERE discord_user_id=? AND rank<=3",(uid,)).fetchone()[0]>0),("season_winner","Vainqueur saison",c.execute("SELECT count(*) FROM industrial_season_rewards WHERE discord_user_id=? AND rank=1",(uid,)).fetchone()[0]>0),("join_team","Rejoindre une équipe",c.execute("SELECT count(*) FROM industrial_team_members WHERE discord_user_id=? AND role<>'owner'",(uid,)).fetchone()[0]>0),("team_action","Première action en équipe",c.execute("SELECT count(*) FROM industrial_team_audit WHERE actor_discord_user_id=? AND action NOT IN('invite','accepted','declined')",(uid,)).fetchone()[0]>0))
            for key,title,earned in seasonal:
                if earned:c.execute("INSERT OR IGNORE INTO industrial_achievements(discord_user_id,achievement_key,title,reputation_awarded) VALUES(?,?,?,0)",(uid,key,title))
    ...
    def get_notification_preferences(self,uid):
        with self._read() as c:
            row=c.execute("SELECT * FROM industrial_notification_preferences WHERE discord_user_id=?",(uid,)).fetchone();return dict(row) if row else {"enabled":1,"market_enabled":1,"transport_enabled":1,"forge_enabled":1,"shipment_enabled":1,"contract_enabled":1,"season_enabled":1,"event_enabled":1,"team_enabled":1}

    def set_notification_preference(self,uid,category,enabled):
        columns={"all":"enabled","market":"market_enabled","transport":"transport_enabled","forge":"forge_enabled","shipment":"shipment_enabled","contract":"contract_enabled","season":"season_enabled","event":"event_enabled","team":"team_enabled"}
    ...
    def get_admin_log(self,uid,limit=20):
        with self._read() as c:
            actor=c.execute("SELECT id FROM industrial_actors WHERE discord_user_id=?",(uid,)).fetchone();aid=int(actor[0]) if actor else -1
            sql="SELECT transaction_type,monetary_effect,credits,resource_type,quantity,reference_type,reference_id,metadata,created_at FROM industrial_transactions WHERE actor_id=? OR counterparty_actor_id=? OR json_extract(metadata,'$.target_discord_user_id')=? UNION ALL SELECT 'season_reward','none',NULL,NULL,NULL,'season',season_id,'{}',granted_at FROM industrial_season_rewards WHERE discord_user_id=? UNION ALL SELECT 'team_'||action,'none',NULL,NULL,NULL,'team',company_id,metadata,created_at FROM industrial_team_audit WHERE actor_discord_user_id=? OR target_discord_user_id=? ORDER BY created_at DESC LIMIT ?"
            return [dict(r) for r in c.execute(sql,(aid,aid,uid,uid,uid,uid,limit))]

    def economy_check(self):
        with self._read() as c:
            integrity=c.execute("PRAGMA integrity_check").fetchone()[0];foreign=c.execute("PRAGMA foreign_key_check").fetchall();queries={"Wallets":"SELECT count(*) FROM industrial_users WHERE credits<0","Inventaires":"SELECT count(*) FROM industrial_inventory WHERE quantity<0","Escrows":"SELECT count(*) FROM industrial_market_orders WHERE escrow_credits<0 OR escrow_quantity<0","Marche":"SELECT count(*) FROM industrial_market_orders WHERE remaining_quantity<0 OR remaining_quantity>original_quantity","Transports":"SELECT count(*) FROM industrial_transports WHERE arrival_at<departure_at OR current_duration_seconds<0","Forge":"SELECT count(*) FROM industrial_forge_jobs WHERE output_quantity<=0 OR finishes_at<started_at","Contrats":"SELECT count(*) FROM industrial_contracts WHERE escrow_credits<0 OR (status='open' AND escrow_credits<>total_price)","Acteurs":"SELECT count(*) FROM industrial_actors a LEFT JOIN industrial_users u ON u.discord_user_id=a.discord_user_id LEFT JOIN industrial_ai_companies ai ON ai.id=a.ai_company_id WHERE (a.actor_type='player' AND u.discord_user_id IS NULL) OR (a.actor_type='ai' AND ai.id IS NULL)","Request IDs":"SELECT count(*) FROM (SELECT request_id,count(*) n FROM industrial_market_orders GROUP BY request_id HAVING n>1)","Saisons":"SELECT max(0,count(*)-1) FROM industrial_seasons WHERE status='active'","Scores saison":"SELECT count(*) FROM industrial_season_scores s LEFT JOIN industrial_seasons x ON x.id=s.season_id LEFT JOIN industrial_actors a ON a.id=s.actor_id WHERE x.id IS NULL OR a.id IS NULL","Titres":"SELECT count(*) FROM industrial_user_titles ut LEFT JOIN industrial_titles t ON t.id=ut.title_id WHERE t.id IS NULL","Titres équipés":"SELECT count(*) FROM (SELECT discord_user_id,count(*) n FROM industrial_user_titles WHERE equipped=1 GROUP BY discord_user_id HAVING n>1)","Evénements":"SELECT count(*) FROM industrial_economic_events WHERE multiplier_bps NOT BETWEEN 8000 AND 12500 OR ends_at<=starts_at","Membres équipe":"SELECT count(*) FROM industrial_team_members tm LEFT JOIN industrial_companies co ON co.id=tm.company_id WHERE co.id IS NULL","Owners équipe":"SELECT count(*) FROM industrial_companies co WHERE NOT EXISTS(SELECT 1 FROM industrial_team_members tm WHERE tm.company_id=co.id AND tm.role='owner')","Invitations équipe":"SELECT count(*) FROM industrial_team_invitations WHERE inviter_discord_user_id=invitee_discord_user_id OR expires_at<=created_at"};checks={"SQLite":0 if integrity=="ok" else 1,"Foreign keys":len(foreign)};checks.update({k:int(c.execute(q).fetchone()[0]) for k,q in queries.items()});return {"integrity":integrity,"foreign_key_errors":[tuple(r) for r in foreign],"checks":checks}
